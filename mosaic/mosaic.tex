...
\author{\textbf{M. Schenck, P. Gevorgyan, S. Jain,}\\ \textbf{D. Kumar Nath, S. Khedar, B. Bollen}\\ for OpenST Foundation \\ preliminary draft}
...
Before a logout message for a validator is included the end height is considered to be $n_e = \infty$.
...
Hash lock $h_l$ is a hash that is set by the facilitator\footnote{ refer facilitator section}, refer section 4.2.4

\begin{align}
  M = (i, n, g_p, g_l, s, h_l) 
\end{align}

Message hash $m_h$ is an unique identifier of a message $M$. It can be calculated as the hash of message type hash\footnote{For more details of facilitator please refer section 4.5} $t$, intent hash $i$ , nonce $n$, gas price $g_p$ , gas limit $g_l$ ,sender $s$ and $K$ is $keccak256$ hashing function. 
...
\subsection*{Progress with HashLock} 
...
Both the contracts are inactive by default i.e both the contracts cannot transfer any messages except linking messages.
 ...
 As explained in section 4.1, each message contains message type intent hash. 
...
As described in section 4.2, message hash generation needs message type specific intent hash which is staking intent hash $i_s$ in this scenario . We define staking intent $i_s$  as : 
 \begin{align}
 	s = K(a_s,b_a,s_a,n_s,g_p,g_l,t_a )
 \end{align}
                                                       
Where $K$ is $keccak256$ hashing function, $a_s$ is staking amount, $b_a$ is beneficiary address, $s_a$ is staker address, $n_s$ is staker nonce, $g_p$ and $g_l$ are gas price and gas limit respectively as described in section 4.8, $t_a$ is ERC20 staking token address. 
...
Facilitator must stake bounty to gateway during message declaration on source chain as described in section 4.7. 
...
On successful mint of token, reward is awarded to facilitator from the minted token refer 4.8 section reward.
...
If message initiator(staker or redeemer) decides to revoke message transfer, it has to pay penalty described in 4.7. 
...
Section 4.1 explains the four step process to transfer message from source to target blockchain. 
...
\subsection{Gateway deactivating}
