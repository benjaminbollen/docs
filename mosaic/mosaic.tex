...
$T^A$ is tracked on the auxiliary chain with every block and consists of the lates observed $f(T^O)$, the accumulated transaction root, the accumulated gas consumed, and the current dynasty on auxiliary.
...
The dynasty number equals the number of finalised checkpoints in the chain from the root checkpoint to the parent block, as per \cite{casperffg}.
...
For a meta-blockchain $\mathcal{B}$ the chain can be appended by proposing and committing new meta-blocks $B_i(K, T, S)$ in the core contract on the origin block\-chain. The genesis meta-block $B_0$ is considered committed.

Given a committed meta-block $B_n$ at height $n$ a proposal for a meta-block $B_{n+1}(K_{n+1}, T_{n+1}, \cdot)$ at height $n+1$ is a valid proposal if the validity assertions for the kernel $K_{n+1}$ hold and the 
