   ...
   % \end{umlcall}
    ...
    \begin{umlfragment}[type=alt, label=accept, name=policy, inner xsep=2]
      ...
      \begin{umlcall}[op={: acceptStakeRequest(staker, hashLock)}]{Facilitator}{Branded Token Gate}  %return={ emit StakingRequestAccpeted (staker, amountST, amountUT, nonce, unlockHeight, stakingIntentHash}
        ...
        \begin{umlcall}[op={: stake(uuid, amountST, beneficiary, hashLock, staker)}]{Branded Token Gate}{OpenSTValue} % return={amountUT, nonce, unlockHeight, stakingIntentHash}
          ...
          \begin{umlcall}[type=return, op={emit StakingIntentDeclared(uuid, staker, nonce, beneficiary, amountST, amountUT, unlockHeight, StakingIntentHash, ChainIdUtility)}]{OpenSTValue}{Facilitator}
          \end{umlcall}
        \end{umlcall}
    ...
    \begin{umlfragment}[type=alt, label=proceed, name=phasetwo, inner xsep=1.5]
      \begin{umlcall}[dt=6, fill=green!20, op={: processStaking(StakingIntentHash, unlockSecret)}]{Facilitator}{Branded Token Gate}
        ...
        %\end{umlcallself}
        % check msg.sender = facilitator
        \begin{umlcallself}[op={require(msg.sender = facilitator)}]{Branded Token Gate}
        \end{umlcallself}        
