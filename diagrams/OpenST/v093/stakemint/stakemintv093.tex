...
\begin{tikzpicture}
  \begin{umlseqdiag}
   ...
   % \umlboundary[class=SK, fill=blue!40]{Token Holder} %Token Holder contract in v0.9.4
   ...
   % \end{umlcall}
    ...
    \begin{umlfragment}[type=alt, label=accept, name=policy, inner xsep=2]
      ...
      \begin{umlcall}[op={: acceptStakeRequest(staker, hashLock)}]{Facilitator}{Branded Token Gate}  %return={ emit StakingRequestAccpeted (staker, amountST, amountUT, nonce, unlockHeight, stakingIntentHash}
        ...
        \begin{umlcall}[op={: stake(uuid, amountST, openSTValue, hashLock, brandedToken)}]{Branded Token Gate}{OpenSTValue} % return={amountUT, nonce, unlockHeight, stakingIntentHash}
          ...
          \begin{umlcall}[type=return, op={emit StakingIntentDeclared(uuid, brandedToken, nonce, openSTvalue, amountST, amountUT, unlockHeight, StakingIntentHash, ChainIdUtility)}]{OpenSTValue}{Facilitator}
          \end{umlcall}
        \end{umlcall}
      ...
      \begin{umlcall}[dt=5, op={: rejectStakeRequest(staker, reason)}]{Facilitator}{Branded Token Gate}
        ...
        \begin{umlcall}[fill=green!20, op={: transfer(staker, amountST)}]{Branded Token Gate}{OST}
          ...
          \begin{umlcall}[dt=10, type=return, op={emit StakeRequestRejected(staker, amountST, reason)}]{Branded Token Gate}{Staker}
          \end{umlcall}
        \end{umlcall}
      \end{umlcall}
      \end{umlcall}
    ...
    \begin{umlfragment}[type=loop, name=mosaic]
      ...
      \begin{umlcall}[dt=180, op={new block(blockHeight, stateRoot)}]{Utility Chain}{Foundation}
    ...
    \begin{umlfragment}[type=alt, label=proceed, name=phasetwo, inner xsep=0.5]
      ...
      \begin{umlcall}[op={: processMinting(StakingIntentHash, unlockSecret)}]{Facilitator}{OpenSTUtility}
        ...
        \end{umlcallself}
        % mint Branded Tokens
        \begin{umlcall}[fill=green!20, op={: mint(brandedToken, amountUT)}]{OpenSTUtility}{Branded Token}
          % store claim
          \begin{umlcallself}[op={store claim}]{Branded Token}
          \end{umlcallself}
        \end{umlcall}
        \begin{umlcall}[type=return, op={emit ProcessedMint(uuid, StakingIntentHash, tokenAddress, staker, brandedToken, amountUT, unlockSecret)}]{OpenSTUtility}{Hunter}
        \end{umlcall}
      \end{umlcall}
      ...
      \begin{umlcall}[dt=5, op={: claim(beneficiary)}]{Facilitator}{Branded Token}
       % \begin{umlcall}[fill=green!20, type=return, op={<<UT(amountUT)>>}]{Branded Token}{Branded Token Gate}
       % \end{umlcall}
      \end{umlcall}
