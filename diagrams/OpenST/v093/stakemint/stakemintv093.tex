...
\begin{tikzpicture}
  \begin{umlseqdiag}
    ...
    % set workers for branded token
      \begin{umlcall}[dt=5,op={: setWorker(worker, deactivationHeight)}]{Staker}{Workers}  
      \end{umlcall}    
    
    % approve workers to spend bounty amount
      \begin{umlcall}[dt=10,op={: approve(worker, bounty)}]{OST}{Workers}  
      \end{umlcall}    
          
    % staker requests stake to gate
    \begin{umlcall}[dt=10, op={: requestStake(amount, gate)}]{Staker}{Branded Token Gate}
      % gate pulls amount from staker on OST
      \begin{umlcall}[fill=green!20, dt=5, op={: transferFrom(staker, gate, amount)}, return=<<OST(amount)>>]{Branded Token Gate}{OST}
      ...
      \begin{umlcall}[dt=5, type=return, op={emit StakeRequested(staker, amount, gate)}]{Branded Token Gate}{Facilitator}
      \end{umlcall}
    \end{umlcall}
