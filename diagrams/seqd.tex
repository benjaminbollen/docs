...
\begin{tikzpicture}
  \begin{umlseqdiag}
    ...
    \end{umlfragment}
    \umlnote[x=25, y=-22]{mosaic}{Placeholder for OpenST Mosaic game}
    ...
    \begin{umlcall}[dt=5, op={: ConfirmStakingIntent(StakingIntentHash, uuid, hashLock, amountST, amountUT, stakingProcessor, stakingProcessorNonce, tokenHolder, unlockHeight)}, return={emit StakingIntentConfirmed(StakingIntentHash, nonce, unlockHeight, expirationHeight)}]{Staking Processor}{OpenSTUtility}
    ...
    \begin{umlfragment}[type=alt, label=proceed, name=phasetwo, inner xsep=6]
      % Staking Processor first calls processStaking to ensure the bounty is return to him
      \begin{umlcall}[dt=6, fill=green!20, op={: processStaking(StakingIntentHash, unlockSecret)}]{Staking Processor}{OpenSTValue}
        ...
        \begin{umlcall}[fill=green!20, op={: transfer(SimpleStake, amount)}]{OpenSTValue}{OST}
          ...
          \end{umlcall}
        \end{umlcall}
        % transfer bounty OST to Staking Processor
        \begin{umlcall}[fill=green!20, op={: transfer(processor, bounty)}]{OpenSTValue}{OST}
          \begin{umlcall}[fill=green!20, type=return, op={<<OST(bounty)>>}]{OST}{Staking Processor}
          \end{umlcall}
        \end{umlcall}
      ...
      % Staking Processor then calls processMinting to mint the utility tokens
      \begin{umlcall}[op={: processMinting(StakingIntentHash, unlockSecret)}]{Staking Processor}{OpenSTUtility}
      ...
      \begin{umlfragment}[type=opt, name=bounty]
        \begin{umlcall}[dt=6, fill=green!20, op={: processStaking(StakingIntentHash, unlockSecret)}]{Hunter}{OpenSTValue}
          % transfer amount OST to SimpleStake
          \begin{umlcall}[fill=green!20, op={: transfer(SimpleStake, amount)}]{OpenSTValue}{OST}
            \begin{umlcall}[fill=green!20, type=return, op={<<OST(amount)>>}]{OST}{SimpleStake}
            \end{umlcall}
        ...
        \end{umlcall}
      \end{umlfragment}
      \umlnote[x=12, y=-52]{bounty}{If stake was left unprocessed, unlock secret is known through mint.}
      
      \umlfpart[revert]
      \begin{umlcall}[dt=30, op={: revertStaking(StakingIntentHash)}]{Staking Processor}{OpenSTValue}
