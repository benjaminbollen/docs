...
\begin{tikzpicture}
  \begin{umlseqdiag}
    ...
    \begin{umlfragment}[type=alt, label=accept, name=policy, inner xsep=2]
      ...
      \begin{umlcall}[dt=6, op={: acceptRequest(staker, amount, hashLock)}]{Facilitator}{Branded Token Gate}  
        ...
        \begin{umlcall}[op={: stake(uuid, amount, hashLock, tokenHolder)}, return={nonce, unlockHeight}]{Branded Token Gate}{OpenSTValue}
          ...
          \begin{umlcall}[dt=4, op={: transferFrom(facilitator, OpenSTValue, amount+bounty)}, fill=green!20, return=<<OST(amount+bounty)>>]{OpenSTValue}{OST}
            % pull OST from Facilitator
            \begin{umlcall}[dt=10, fill=green!20, type=return]{Facilitator}{OST}
            \end{umlcall}
          \end{umlcall}
          ...
          \begin{umlcallself}[dt=0, op={HTLC(facilitator, amount+bounty)},]{OpenSTValue}
          \end{umlcallself}
    ...
    \umlnote[x=30,y=-8, width=200]{policy}{to accept the staking request the facilitator generates a secret, random unlockSecret, and publishes hashLock=Hash(unlockSecret)}
    ...
    \begin{umlfragment}[type=loop, name=mosaic]
      ...
      \begin{umlcall}[dt=145, op={new block(blockHeight, stateRoot)}]{Utility Chain}{Foundation}
      ...
      \begin{umlcall}[dt=175, op={new block(blockHeight, stateRoot)}]{Value Chain}{Foundation}
    ...
    \end{umlfragment}
    \umlnote[x=25, y=-24, width=180]{mosaic}{Placeholder for OpenST Mosaic game}
    ...
    \begin{umlfragment}[type=alt, label=proceed, name=phasetwo, inner xsep=6]
      ...
      \begin{umlcall}[dt=6, fill=green!20, op={: processStaking(StakingIntentHash, unlockSecret)}]{Facilitator}{OpenSTValue}
        % transfer amount OST to SimpleStake
        \begin{umlcall}[fill=green!20, op={: transfer(simpleStake, amount)}]{OpenSTValue}{OST}
          \begin{umlcall}[fill=green!20, type=return, op={<<OST(amount)>>}]{OST}{SimpleStake}
          \end{umlcall}
        \end{umlcall}
        % transfer bounty OST to Facilitator
        \begin{umlcall}[fill=green!20, op={: transfer(facilitator, bounty)}]{OpenSTValue}{OST}
          \begin{umlcall}[fill=green!20, type=return, op={<<OST(bounty)>>}]{OST}{Facilitator}
          \end{umlcall}
        \end{umlcall}
        \begin{umlcall}[type=return, op={emit ProcessedStake(StakingIntentHash, unlockSecret)}]{OpenSTValue}{Hunter}
        \end{umlcall}
      \end{umlcall}
      ...
      \begin{umlfragment}[type=parallel]
        \begin{umlfragment}[type=opt, name=bounty, label={ensure completion}, inner xsep=15]
          \begin{umlcall}[dt=10, fill=green!20, op={: processStaking(StakingIntentHash, unlockSecret)}]{Hunter}{OpenSTValue}
            % transfer amount OST to SimpleStake
            \begin{umlcall}[fill=green!20, op={: transfer(simpleStake, amount)}]{OpenSTValue}{OST}
              \begin{umlcall}[fill=green!20, type=return, op={<<OST(amount)>>}]{OST}{SimpleStake}
              \end{umlcall}
          ...
          \end{umlcall}
        \end{umlfragment}
        \umlnote[x=12, y=-54, width=150]{bounty}{If stake was left unprocessed, unlock secret is known through mint. Bounty is always transferred to msg.sender of processStaking()}
        
        \umlfpart[claim]
        % Facilitator (or anyone) can call claim to transfer UT to token holder
        \begin{umlcall}[dt=37, op={: claim(tokenHolder)}]{Facilitator}{Branded Token}
          \begin{umlcall}[fill=green!20, type=return, op={<<UT(amountUT)>>}]{Branded Token}{Token Holder}
          \end{umlcall}
        \end{umlcall}
        
        \begin{umlcall}[op={: returnRevertedStake(staker, amount)}]{Facilitator}{Branded Token Gate}
      ...
      \begin{umlcall}[dt=5, fill=green!20, op={: revertStaking(StakingIntentHash)}]{Facilitator}{OpenSTValue}
        \begin{umlcallself}[op={require(unlockHeight <= block.number)}]{OpenSTValue}
        \end{umlcallself}
        % transfer amount and bounty OST to facilitator
        \begin{umlcall}[fill=green!20, op={: transfer(facilitator, amount+bounty)}]{OpenSTValue}{OST}
          \begin{umlcall}[fill=green!20, type=return, op={<<OST(amount + bounty)>>}]{OST}{Facilitator}
          \end{umlcall}
        \end{umlcall}
      \end{umlcall}
      
      % Facilitator (or anyone) can revert 
      \begin{umlcall}[op={: returnRevertedStake(staker, amount)}]{Facilitator}{Branded Token Gate}
        % check unlockHeight is in the past
        \begin{umlcallself}[op={require(unlockHeight <= block.number)}]{Branded Token Gate}
        \end{umlcallself}
      ...
      \end{umlcall}
    \end{umlfragment}
    \umlnote[x=0, y=-50, width=100]{phasetwo}{With both value chain and utility chain configured correctly for StakingIntentHash, Facilitator can proceed by revealing the unlock secret, or revert by awaiting the unlock height }
    
    \begin{umlfragment}
      \begin{umlcall}[dt=12, fill=green!20, op={: returnRevertedStake(staker, amount)}]{Staker}{Branded Token Gate}
        % check unlockHeight is in the past
        \begin{umlcallself}[op={require(unlockHeight <= block.number)}]{Branded Token Gate}
        \end{umlcallself}
        \begin{umlcall}[fill=green!20, op={: transfer(staker, amount)}]{Branded Token Gate}{OST}
