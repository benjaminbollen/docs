...
\begin{tikzpicture}
  \begin{umlseqdiag}
      ...
      \begin{umlcall}[dt=6, op={: acceptRequest(staker, amount, hashLock)}]{Facilitator}{Branded Token Gate}  
        ...
        \begin{umlcall}[op={: stake(uuid, amount, hashLock, tokenHolder)}, return={nonce, unlockHeight}]{Branded Token Gate}{OpenSTValue}
          ...
          \begin{umlcall}[dt=4, op={: transferFrom(facilitator, OpenSTValue, amount)}, fill=green!20, return=<<OST(amount)>>]{OpenSTValue}{OST}
            ...
            \begin{umlcall}[dt=20, fill=green!20, type=return]{Facilitator}{OST}
            \end{umlcall}
          \end{umlcall}
    ...
    \begin{umlfragment}[type=loop, name=mosaic]
      ...
      \begin{umlcall}[dt=170, op={new block(blockHeight, stateRoot)}]{Utility Chain}{Foundation}
      ...
      \begin{umlcall}[dt=200, op={new block(blockHeight, stateRoot)}]{Value Chain}{Foundation}
    ...
    \begin{umlfragment}[type=alt, label=proceed, name=phasetwo, inner xsep=6]
      ...
      \begin{umlcall}[dt=20, fill=green!20, op={: revertStaking(StakingIntentHash)}]{Facilitator}{Branded Token Gate}
        ...
        \begin{umlcall}[fill=green!20, op={: revertStaking(StakingIntentHash)}]{Branded Token Gate}{OpenSTValue}
          ...
          \begin{umlcall}[fill=green!20, op={: transfer(facilitator, amount)}]{OpenSTValue}{OST}
            \begin{umlcall}[fill=green!20, type=return, op={<<OST(amount)>>}]{OST}{Facilitator}
            \end{umlcall}
          \end{umlcall}
        \end{umlcall}
        % transfer amount of staker back to staker from gate
        \begin{umlcall}[fill=green!20, op={: transfer(staker, amount)}]{Branded Token Gate}{OST}
          \begin{umlcall}[type=return, fill=green!20, op={<<OST(amount)>>}]{OST}{Staker}
          \end{umlcall}
        \end{umlcall}
