...
\begin{theorem}[Leveraged Security]
	...
	\begin{proof}
		...
		Lemma~\ref{lemma:remoteaccountability} proves that the validators can be slashed or Origin for their actions on Auxiliary.
...
% Introduce OSTblock
%
% gas limit means that if not the next finalised block is reported then anyone can blame on an older finalised block above the limit and all validators will be punished
% we need to track all gas consumption
% validators get paid per gas

\begin{theorem}[Honesty Guard]
	\label{theorem:honestyguard}
	An honest validator will not be punished by the OSTgas limit rules.
	\begin{proof}
